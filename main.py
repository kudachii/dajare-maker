...
def init_gemini():
    try:
        ...
        # 2. モデルの選択 (NotFoundエラー回避のため models/ を明示)
        # 2025年現在、最も安定している名称を使用
...
with st.sidebar:
    ...
    st.write("---")
    st.caption("Ver 1.0.1 (Stable Build)")
...
if error_msg and "GEMINI_API_KEY" not in st.secrets and "temp_api_key" not in st.session_state:
    st.warning("👈 サイドバーからAPIキーを入力して開始してください。")
else:
        ...
        if st.button("審査員に提出"):
            if user_input and model:
                with st.spinner('審査員が真剣に評価中...'):
                    prompt = f"""
                    ユーザーのダジャレ「{user_input}」を判定し、以下の形式で答えてください。
                    【座布団】（0〜5枚の絵文字で評価）
                    【周囲の温度】（マイナス273度〜100度の間で表現）
                    【師匠のコメント】（毒舌かつユーモアのあるコメント）
                    """
                    try:
                        res = model.generate_content(prompt)
...
# --- フッター ---
st.write("---")
st.caption("© 2025 ダジャレ・ラボ | 収益化に向けたプロトタイプ公開中")
