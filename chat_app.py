...
with st.sidebar:
    ...
    if st.button("🚀 LIVEスタート！"):
        if model and user_input:
            ...
            以下の内容で、視聴者がハラハラ・爆笑するような「会話劇」を書き上げてください。
            ...
            【番組の進行ルール（厳守）】:
            1. [オープニング]: 司会（Gemini）が元気に、あるいは「主催者モード」なら少し緊張気味に開始を宣言し、お題を紹介。
            2. [メンター陣の採点]: 5人のメンターが順に発言。主催者モードの場合は、普段の優しさを捨てて、プロの視点から厳しくダメ出しし、最後に「〇〇点（低得点）」を出す。
            3. [平均点発表]: 司会（Gemini）が「恐ろしい結果が出ました…」等と実況しながら、5人の平均点を計算して発表する。
            4. [師匠の総評]: 辛口師匠が平均点すら甘いと一喝し、江戸っ子の毒舌でオチをつけつつ「俺の評価は〇〇点だ！」と引導を渡す。
            5. [エンディング]: 司会（Gemini）がボロボロになったくだちいさんをフォローしたり、あるいは突き放したりして、明るく次回の放送に繋げる。
